...
export default function PageFacultyTeachers() {
    ...
    useEffect( () => {
        window.fetch(`https://schedule.dxrk.dev/api/university/${idUniversity}/teachers`)
        .then(res => res.json())
        .then(
            result => {
                setIsLoaded(true);
                    ...
                    <div className="page-one-university__row">
                        ...
                        <div className="page-faculties__content">
                            {faculties ? faculties.sort((a, b) => a.short_name > b.short_name ? 1 : -1).map(faculty => (
                                <Link key={faculty.id} to={`/university-${idUniversity}/for-teacher/teacher-${faculty.id}`} className="page-faculties__box" title={faculty.full_name}>{faculty.short_name}</Link>
                            )) : null}
                        </div>
                        </div>
                    </div>
