...
export default function PageGroupStudents() {
    ...
    useEffect( () => {
        window.fetch(`https://schedule.dxrk.dev/api/department/${idDepartament}/groups`)
        .then(res => res.json())
        .then(
            result => {
                setIsLoaded(true);
            ...
            <div className="page__wrapper">
                ...
                <main className="page__main">
                    ...
                    <div className="page-one-university__row">
                            <Link to={`/university-${idUniversity}/for-student/faculty-${idFaculty}`} className="page-one-university__button-prev">Вернуться назад</Link>
                        ...
                        <input onInput={event => SearchInput(event.target.value)} className="page-faculties__search" placeholder="Поиск группы..."></input>
                        ...
                        <div className="page-faculties__content">
                            {groups ? groups.sort((a, b) => a.short_name > b.short_name ? 1 : -1).map(group => (
                                <Link key={group.id} to={`/university-${idUniversity}/for-student/faculty-${idFaculty}/departament-${idDepartament}/group-${group.id}`} className="page-faculties__box" title={group.full_name}>{group.short_name}</Link>
                            )) : null}
                        </div>
                        </div>
                    </div>
